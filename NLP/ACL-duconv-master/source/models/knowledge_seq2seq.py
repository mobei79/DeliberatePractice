...
def bidirec_gru(input, input_size, hidden_size,
    ...
    def gru_fun(gru_in, name=None, is_reverse=False):
        ...
        for i in range(num_layers):
            ...
            if is_reverse:
                #将对输入的LoDTensor，在最后一层lod_level上，
                # 选取batch每个序列（sequence）的第一个时间步（time_step）的特征向量作为池化后的输出向量。
                fw_last_hidden = layers.sequence_first_step(fw_gru_out)
            else:
                #将对输入的LoDTensor，在最后一层lod_level上，
                # 选取batch每个序列（sequence）的最后一个时间步（time_step）的特征向量作为池化后的输出向量。
...
def knowledge_seq2seq(config):
    ...
    if bi_direc:
        rnn_hidden_size //= 2
    #基于bi-direction GRU 的utterance encoder 
    #TODO： 注意只实现了单步？？
    ...
    #基于bi-direction GRU 的KG encoder 
    ...
    #cue_att为使用softmax 变化为知识选取的先验概率分布p(k/x) 
    # weighted_cue为cue_att作用下的KG（即论文中kg期望值 KC）
    weighted_cue, cue_att = dot_attention(query, cue_memory, mask=memory_mask)

    cue_att = layers.reshape(cue_att, shape=[batch_size, -1])
    #选用先验概率，得到论文中先验kg期望值 KC
    knowledge = weighted_cue
    #train 时为true，用选取知识的后验概率，test时没有后验概率只能用机器自身学习到的先验概率
