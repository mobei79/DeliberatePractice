...
#TODO：每个data为batchsize组 source_pad_ids target_pad_ids knowledge_pad_ids 即对应的length组成的六个np.array
...
def build_data_feed(data, place,
    ...
    """ build data feed """
    src_ids, src_len, trg_ids, trg_len, kn_ids, kn_len = data
    #src_ids.shape:(-1, pad_source)正常来说src_ids.shape[0]=batch_size
    real_size = src_ids.shape[0]
    if src_ids.shape[0] < batch_size:
        if not is_training:
            #TODO:目前估计是指测试时最后一个batch可能不足batchsize个输入
            src_ids, src_len, trg_ids, trg_len, kn_ids, kn_len = \
                pad_to_bath_size(src_ids, src_len, trg_ids, trg_len, kn_ids, kn_len, batch_size)
        else:
            return None#something wrong program will break
    #扩展维数src_ids从(batch_size, pad_source)为(batch_size, pad_source，)
